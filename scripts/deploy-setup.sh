#!/bin/bash
# One-time GitHub Pages setup for Cute Kicks. Run AFTER `gh auth login --web`.
# Creates a public repo named cute-kicks, pushes, and enables Pages.
set -euo pipefail
...
if ! "$GH" repo view "$LOGIN/cute-kicks" >/dev/null 2>&1; then
  "$GH" repo create cute-kicks --public --description "Personal fetal movement tracker PWA" \
    --source . --remote origin
else
  git remote get-url origin >/dev/null 2>&1 || \
    git remote add origin "https://github.com/$LOGIN/cute-kicks.git"
fi
...
"$GH" api "repos/$LOGIN/cute-kicks/pages" -X POST \
  -f build_type=legacy -f 'source[branch]=main' -f 'source[path]=/' \
  >/dev/null 2>&1 || true

echo
echo "Done. The app will be live in a minute or two at:"
echo "  https://$LOGIN.github.io/cute-kicks/"
